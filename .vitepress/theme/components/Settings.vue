  ...
  <div class="settings">
    ...
    >
      <div class="set-list">
        ...
        <div class="set-item">
          ...
          <div class="set-options">
            <span
              ...
              @click="fontFamily = 'hmos'; store.updateFontClass()"
            >
              HarmonyOS Sans
            </span>
            <span
              :class="['options', { choose: fontFamily === 'ysk' }]"
              @click="fontFamily = 'ysk'; store.updateFontClass()"
            >
              云深醉歌行楷
            </span>
            <span
              :class="['options', { choose: fontFamily === 'lxgw' }]"
              @click="fontFamily = 'lxgw'; store.updateFontClass()"
            >
              霞鹜文楷
            </span>
          </div>
        ...
        <span class="title">播放器配置</span>
        <div class="set-item">
          <span class="set-label">音乐平台</span>
          <div class="set-options">
            <span
              :class="['options', { choose: musicPlatform === 'netease' }]"
              @click="(musicPlatform = 'netease'), (musicId = 5314875708)"
            >
              网易云
            </span>
            <span
              :class="['options', { choose: musicPlatform === 'tencent' }]"
              @click="(musicPlatform = 'tencent'), (musicId = 1171747653)"
            >
              QQ音乐
            </span>
          </div>
        ...
        <div class="set-item">
          <span class="set-label">歌单 ID</span>
          <div class="set-options">
            <input
              v-model="musicId"
              type="number"
        ...
        <div class="set-item">
          <span class="set-label">背景类型</span>
          <div class="set-options">
            <span
              :class="['options', { choose: backgroundType === 'patterns' }]"
              @click="backgroundType = 'patterns'"
        ...
        <div v-if="backgroundType === 'image'" class="set-item">
          <span class="set-label">背景链接（图片或视频）</span>
          <div class="set-options">
            <input
              v-model="backgroundUrl"
              type="text"
              placeholder="请输入图片URL或MP4视频URL"
            />
          </div>
        </div>
...
const { themeType, fontFamily, fontSize, infoPosition, backgroundType, backgroundUrl, bannerType, musicPlatform, musicId } =
  storeToRefs(store);
</script>
