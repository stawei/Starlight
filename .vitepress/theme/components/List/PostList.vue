...
const showCover = (item) => {
  const isEnabled = themeConfig.value?.cover?.showCover?.enable
  if (!isEnabled) return false
  // 如果文章有封面或配置了默认封面，则显示
  const hasItemCover = item?.cover && item.cover !== ''
  const hasDefaultCover = themeConfig.value?.cover?.showCover?.defaultCover?.length > 0
  return hasItemCover || hasDefaultCover
}

// 获取封面图片 按优先级获取：cover > defaultCover > false
const getCover = ({ cover: itemCover, title, regularPath }, index = 0) => {
  const { cover } = themeConfig.value ?? {}

  if (!cover?.showCover?.enable) return false
  if (itemCover) {
    // 如果是随机API且没有指定设备类型，根据当前设备添加
    if (itemCover === 'https://api.elaina.cat/random/') {
      const deviceType = typeof window !== 'undefined' && window.innerWidth <= 768 ? 'mobile' : 'pc';
      return `https://api.elaina.cat/random/${deviceType}?t=${Date.now()}`;
    }
    return itemCover;
  }

  // 使用文章标题或路径生成固定索引，确保同一文章始终显示相同封面
  const defaultCovers = cover.showCover.defaultCover
  if (Array.isArray(defaultCovers) && defaultCovers.length > 0) {
    // 使用文章标题+路径组合作为种子，生成更分散的哈希值
    const seed = (title || '') + (regularPath || '')
    // 简化的哈希算法 - 性能更好且分布均匀
    let hash = 0
    ...
    const hashIndex = Math.abs(hash) % defaultCovers.length
    // 结合列表索引，让相邻文章使用不同的图片
    return defaultCovers[(hashIndex + index) % defaultCovers.length]
  }
  return false
}
