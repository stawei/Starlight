    ...
    <nav :class="['main-nav', scrollData.direction, { top: scrollData.height === 0 }]">
      <div class="nav-all">
          ...
          <div class="site-menu">
            <div v-for="(item, index) in theme.nav" :key="index" class="menu-item">
              ...
              <div v-if="item.items" class="link-child">
                ...
                >
                  <i v-if="child.icon === 'heart'" class="ri ri-hearts-fill" />
                  <i v-else-if="child.icon === 'seedling'" class="ri ri-seedling-fill" />
                  <i v-else-if="child.icon" :class="`iconfont icon-${child.icon}`" />
                  {{ child.text }}
                </span>
              </div>
            </div>
        ...
        <div class="right-nav">
          ...
          <!-- 搜索 -->
          <div
            v-if="theme.search.enable && !isMusicPage"
            class="menu-btn nav-btn"
            title="全站搜索"
...
const isMusicPage = computed(() => {
  ...
  return routePath === "/pages/music" || routePath === "/pages/music/";
});
</script>
