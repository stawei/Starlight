...
const postMetaData = computed(() => {
  // 确保路径格式一致：添加 .md 后缀以匹配 getPostData.mjs 中的路径格式
  const relativePath = page.value.relativePath.endsWith('.md')
    ? page.value.relativePath
    : `${page.value.relativePath}.md`;
  const postId = generateId(relativePath);
