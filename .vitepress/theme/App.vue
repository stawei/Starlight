<template>
  ...
  <!-- 页脚 -->
  <FooterLink v-if="!isMusicPage" to="body" />  
  <Footer v-if="!isMusicPage" to="body" />
  <!-- 悬浮菜单（音乐馆页面隐藏） -->
  <Teleport v-if="!isMusicPage" to="body">
    <!-- 左侧菜单 -->
...
const isMusicPage = computed(() => {
  const routePath = decodeURIComponent(route.path);
  return routePath.includes('/music') && !routePath.includes('/posts/');
});
