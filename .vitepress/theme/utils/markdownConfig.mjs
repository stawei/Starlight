...
const markdownConfig = (md, themeConfig) => {
  ...
  // 图片 - 优化：添加 decoding="async" 提升性能，使用 encodeURI 防止特殊字符问题
  md.renderer.rules.image = (tokens, idx) => {
    const token = tokens[idx];
    const srcAttr = token.attrs[token.attrIndex("src")];
    const src = srcAttr ? encodeURI(srcAttr[1]) : '';
    const alt = token.content || '';
    const encodedAlt = alt.replace(/"/g, '&quot;');

    if (!themeConfig.fancybox.enable) {
      return `<img src="${src}" alt="${encodedAlt}" loading="lazy" decoding="async">`;
    }
    return `<a class="img-fancybox" href="${src}" data-fancybox="gallery" data-caption="${encodedAlt}">
                <img class="post-img" src="${src}" alt="${encodedAlt}" loading="lazy" decoding="async" />
                ${alt ? `<span class="post-img-tip">${encodedAlt}</span>` : ''}
              </a>`;
  };
