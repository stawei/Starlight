...
const getPostMDFilePaths = async () => {
  try {
    ...
    // 规范化路径分隔符为正斜杠，兼容 Windows 下的反斜杠，然后只保留 posts 目录下的文件
    const normalized = paths.map((p) => p.replace(/\\\\/g, "/"));
    return normalized.filter((item) => item.includes("posts/"));
  } catch (error) {
    ...
    let posts = await Promise.all(
      paths.map(async (item) => {
        try {
          ...
          // 如果没有封面，使用随机图片API（前端会根据设备类型自动适配）
          const finalCover = cover || 'https://api.elaina.cat/random/';
